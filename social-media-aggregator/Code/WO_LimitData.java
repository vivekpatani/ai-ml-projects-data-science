...
class Crawler{
	...
	//	String input=sc.nextLine();
		...
		for (int m=0; m<seed.length;m++){	
			...
			int d1c,d2c,d3c;
		d1c=d2c=d3c=0;
		for(int i=0;i<urls.size() && i <100 ;i++){
			try{
			URLInfo ui = urls.get(i);	
			URL u = ui.url;
			frequency = 0;
			Scanner in = new Scanner(u.openStream()); //Get the URL stream into Scanner
			System.out.println("("+(i+1)+")"+u.toString()+" and depth:"+ui.depth); //Print the current url
			...
			pqrs:	while((start=s.indexOf("<a href=\"http://",start))!=-1){ //Find links in the page
					...
					if(!checkduplicate(urls,ua.url)){//Check if the found link is not already present in url list
					switch(ua.depth)
					{
						case 1: d1c++;
								if(d1c<=5) urls.add(ua);
								else break pqrs;
								break; 
						case 2: d2c++;
		...
		if(key_urls.size()==0) System.out.println("Keyword not found");
			...
			{ //Print all URLs containing the keyword
				...
				System.out.println("Depth:"+u.depth);
